/* All job titles pulled from http://www.indeed.com/find-jobs.jsp, just grabbing the top job titles for each letter */
var JOB_TITLES = [
  'Unemployed',
  'Account Executive',
...
var submitForm = function(jobtitleContainer, jobtitles) {
  var cities = document.getElementsByClassName('js-city');
  var teams = document.getElementsByClassName('js-team');
  if (cities && cities.length === 1
      && teams && teams.length === 1) {
    var job = JOB_TITLES[(cities[0].value * teams[0].value * 10) % JOB_TITLES.length];
    [].forEach.call(jobtitles, function(jobtitle) {
      jobtitle.textContent = job;
    });
    jobtitleContainer.style.visibility = '';
  }
}

var initSubmitForm = function() {
  var submitBtns = document.getElementsByClassName('js-submit-btn');
  var jobtitleContainers = document.getElementsByClassName('js-jobtitle-container');
  var jobtitles = document.getElementsByClassName('js-jobtitle');
  if (submitBtns && submitBtns.length === 1
      && jobtitleContainers && jobtitleContainers.length === 1
      && jobtitles && jobtitles.length === 2) {
    submitBtns[0].addEventListener('click', submitForm.bind(this, jobtitleContainers[0], jobtitles));
  }
}
